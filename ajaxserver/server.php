...
if (isset($_POST['submit_email'])) {
    ...
    if (!isset($response['error']) || $response['error'] === '') {
        ...
        if (mail($recipient, $subject, $email_content, $email_headers)) {
            // Set a 200 (okay) response code.
            http_response_code(200);
            $response['success'] = "Thank You! haayany will soon be with you.";

        } else {
            // Set a 500 (internal server error) response code.
            //http_response_code(500);
            http_response_code(500);
            $response['error'] = "Oops! Something went wrong and haayany.com couldn't send your message. If this happens again, please contact haayany via haayany@live.co.kr. Thank you!";
